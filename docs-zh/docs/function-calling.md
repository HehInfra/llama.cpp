...
[chat.h](../common/chat.h)（https://github.com/ggml-org/llama.cpp/pull/9639）新增了对 [OpenAI 风格函数调用](https://platform.openai.com/docs/guides/function-calling) 的支持，用于：
- 使用 `--jinja` 标志启动的 `llama-server`

## 通用支持：原生与通用处理程序

所有模型都支持函数调用（详见 https://github.com/ggml-org/llama.cpp/pull/9639）：

- 支持的原生工具调用格式：
  - Llama 3.1 / 3.3（包括内置工具支持——工具名称为 `wolfram_alpha`、`web_search` / `brave_search`、`code_interpreter`），Llama 3.2
...
- 当模板未被原生格式处理程序识别时，支持通用工具调用（日志中会显示 `Chat format: Generic`）。
  - 在适当情况下使用 `--chat-template-file` 覆盖模板（见下文示例）
  - 通用支持可能会消耗更多 token，并且效率低于模型的原生格式。

- 部分模型支持多/并行工具调用，但默认关闭，可在补全端点请求体中传入 `"parallel_tool_calls": true` 来启用。

<details>
<summary>显示一些常用模板及其使用的格式处理程序</summary>
...
<!-- TODO @ngxson：由于 minja 依赖已被移除，我们应当更新此处 -->
...
首先，用任意模型启动服务器，但要确保它使用了支持工具的模板：你可以通过查看 `http://localhost:8080/props` 中的 `chat_template` 或 `chat_template_tool_use` 属性来确认）。

以下是一些已知可用的模型（必要时需覆盖聊天模板）：
...
要从原始 HuggingFace 仓库获取官方模板，可以使用 [scripts/get_chat_template.py](../scripts/get_chat_template.py)（调用示例见 [models/templates/README.md](../models/templates/README.md)）

> [!TIP]
> 如果没有官方的 `tool_use` Jinja 模板，你可以设置 `--chat-template chatml` 以使用一个对许多模型都有效的默认模板（效果因模型而异！），或者编写自己的模板（例如，我们为 DeepSeek R1 蒸馏模型提供了自定义的 [llama-cpp-deepseek-r1.jinja](../models/templates/llama-cpp-deepseek-r1.jinja)）

> [!CAUTION]
> 注意极端的 KV 量化（例如 `-ctk q4_0`），它们可能会显著降低模型的工具调用性能。

在命令行中测试（或使用任何支持 OpenAI 兼容 API 后端的库/软件）：
